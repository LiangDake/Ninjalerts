...
# 导入邮箱账号信息
def email_account_import(csv_file_name, account_list):
    # 读取文件信息
    with open(csv_file_name, 'r') as csv_file:
        ...
        for row in csv_reader:
            # 以每一列为基础，分别为用户名，账号
            email_username, email_password = row
    ...
    print(f'{csv_file_name}邮箱文件导入成功！')
    print()


# 导入账号信息
def twitter_account_import(csv_file_name, account_list):
    # 读取文件信息
    with open(csv_file_name, 'r') as csv_file:
        csv_reader = csv.reader(csv_file)
        # 遍历CSV文件的每一行并添加到列表
        for row in csv_reader:
            # 以每一列为基础，分别为用户名，账号
            twitter_username, twitter_password = row
            # 循环遍历每一列的账号信息，即为新的导入账号信息
            new_account = {
            ...
            # 将每一列新的账号信息都导入至账号数组里
            account_list.append(new_account)
    print(f'{csv_file_name}推特文件导入成功！')
    print()
...
# 邮箱注册新账号
def get_verify_code(email_username, email_password):
    # 点击Create An Account
...
def new_account_signup(verify_code):
    # 输入获得的验证码
...
if __name__ == '__main__':
    ...
    twitter_accounts = {}
    for i in range(20):
        email_account_import('Email.csv', email_account_list)
        twitter_account_import('Twitter.csv', twitter_account_list)
        email_username = email_account_list[0]['Email_Username']
        email_password = email_account_list[0]['Email_Password']
        twitter_username = twitter_account_list[0]['Twitter_Username']
        twitter_password = twitter_account_list[0]['Twitter_Password']
        print(email_username, email_password, twitter_username, twitter_password)
        # 进入推特页面登录账号
        # twitter_login(twitter_username, twitter_password)

        # 通过邀请链接进入页面
        # 确保点击的链接是主账号的邀请链接
        driver.get('https://www.ninjalerts.com/@eduardotejera20')
        time.sleep(3)
        verify_code = get_verify_code(email_username, email_password)
        if verify_code is not False:
            print(verify_code)
            # 邮箱注册新账号成功:
            if new_account_signup(verify_code):
                # 链接Xverse钱包成功:
